...
# 策略名称：适应性移动平均线指标策略
# 关键词：考夫曼适应性移动平均线、均线突破。
# 方法：
# 1)将价格的均值与价格的波动率相结合计算均线；
# 2)当价格高于均线时买入，低于均线时卖出。
...
def initialize(context):
    ...
    # 获取历史数据的长度
    context.user_data.kama_window = 20
...
def handle_data(context):
    # 获取历史数据, 取后window根bar
    hist = context.data.get_price(context.security, count=context.user_data.kama_window * 10,
                                  frequency=context.frequency)
    if len(hist.index) < context.user_data.kama_window * 10:
    ...
    # 计算指标值
    kama = talib.KAMA(close, timeperiod=context.user_data.kama_window)
    ...
    if short_signal_triggered:
        if context.account.huobi_cny_btc >= HUOBI_CNY_BTC_MIN_ORDER_QUANTITY:
            ...
            context.order.sell_limit(context.security, quantity=str(context.account.huobi_cny_btc), price=str(close[-1]*0.999))
        else:
    ...
    # 有买入信号，且持有现金，则全仓买入
        ...
        if context.account.huobi_cny_cash >= HUOBI_CNY_BTC_MIN_ORDER_CASH_AMOUNT:
            ...
            context.order.buy_limit(context.security, quantity=str(context.account.huobi_cny_cash/close[-1]*0.98), price=str(close[-1]*1.001))
        else:
