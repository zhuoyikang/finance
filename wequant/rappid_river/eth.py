...
def initialize(context):
    ...
    # 设置回测基准, 比特币："huobi_cny_eth", 莱特币："huobi_cny_eth", 以太坊："huobi_cny_eth"
    context.benchmark = "huobi_cny_eth"
    # 设置回测标的, 比特币："huobi_cny_eth", 莱特币："huobi_cny_eth", 以太坊："huobi_cny_eth"
    context.security = "huobi_cny_eth"

    context.user_data.period = "60m"
...
def handle_data(context):
    ...
    hist = context.data.get_price(context.security, count=context.user_data.window_long,
                                  frequency=context.user_data.period)
    ...
    if short_mean > upper:
        ...
        if context.account.huobi_cny_cash >= HUOBI_CNY_ETH_MIN_ORDER_CASH_AMOUNT:
            ...
            if buy_quantity < HUOBI_CNY_ETH_MIN_ORDER_QUANTITY:
        ...
        if context.account.huobi_cny_eth >= HUOBI_CNY_ETH_MIN_ORDER_QUANTITY:
            ...
            context.log.warn("卖出数量为 %s" % context.account.huobi_cny_eth)
