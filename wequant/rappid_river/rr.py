...
PARAMS = {
    "start_time": "2017-08-1 00:00:00",  # 回测起始时间
    "end_time": "2017-09-1 00:00:00",  # 回测结束时间
...
def initialize(context):
    ...
    context.period = "1d"
    # 设置回测基准, 比特币："huobi_cny_btc", 莱特币："huobi_cny_ltc", 以太坊："huobi_cny_btc"
    context.benchmark = "huobi_cny_btc"
    # 设置回测标的, 比特币："huobi_cny_btc", 莱特币："huobi_cny_ltc", 以太坊："huobi_cny_btc"
...
def handle_data(context):
    ...
    if short_mean > upper:
        ...
        if context.account.huobi_cny_cash >= HUOBI_CNY_BTC_MIN_ORDER_CASH_AMOUNT:
            ...
            context.log.warn("正在买入 %s 下单金额为 %s 元" % (context.security, context.account.huobi_cny_cash))
        ...
        if context.account.huobi_cny_btc >= HUOBI_CNY_BTC_MIN_ORDER_QUANTITY:
            ...
            context.log.warn("正在卖出 %s 卖出数量为 %s" % (context.security, context.account.huobi_cny_btc))
