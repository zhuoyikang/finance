[[https://wequant.io/study/strategy.atr.html][ATR（真实波幅均值）策略]]
...
def handle_data(context):
    ...
    try:
        ...
        context.log.error("计算 ATR 时出现错误...")
        return

    # 获取最新价格
    current_price = context.data.get_current_price(context.security)
    # 获取 context.user_data.pre_period 个 bar 前的价格
    ...
    context.log.info("当前价格=%s 元, 上轨=%s 元, 下轨=%s 元 prev_price=%s atr=%s" % (current_price, upper, lower, prev_price, atr))

    # 如果当前价格比之前价格低 1 个 ATR，产生卖出信号
