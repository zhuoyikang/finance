...
def initialize(context):
    ...
    context.user_data.sma_window_size = 24
    # 确定当前price可否作为base_price的依据就是当前price是否小于20日均线*price_to_sma_threshold
    context.user_data.price_to_sma_threshold = 1
    ...
    context.user_data.sell4, context.user_data.sell3, context.user_data.sell2, context.user_data.sell1 = 1.8, 1.6, 1.4, 1.2
...
def handle_data(context):
    ...
    # 根据策略调整仓位
    if cash_to_spent > HUOBI_CNY_LTC_MIN_ORDER_CASH_AMOUNT:
        #  市价单买入一定金额
        context.log.info("正在买入 %s" % context.security)
        ...
        #  计算需要卖出的数量，并已市价单卖出
        quantity = min(context.account.huobi_cny_ltc, -1 * cash_to_spent / price)
        if quantity >= HUOBI_CNY_LTC_MIN_ORDER_QUANTITY:
